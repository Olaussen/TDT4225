...
class Queries:
    ...
    def covid_19_tracking(self):
        ...
        for i, res in enumerate(activities_within_60_sec):
            ...
            self.cursor.execute(
                "SELECT * FROM trackpoint WHERE activity_id = {}".format(res[0]))
            a1_trackpoints = self.cursor.fetchall()
            self.cursor.execute(
                "SELECT * FROM trackpoint WHERE activity_id = {}".format(res[1]))
        ...
        print(
            "The users who have been close to another both in time and space are:", user_ids)
